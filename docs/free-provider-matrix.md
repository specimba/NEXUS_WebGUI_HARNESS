...
| 15 | **Pollinations** | **No key at all** | Anonymous free tier, no signup ✅ | Anonymous tier shares IP budgets; occasional queueing 🕐 | OpenAI-Fast class (gpt-oss-20b-ish), rotating | OpenAI-ish `/openai` (non-standard sometimes) | **S** | ✅ yes — `pollinations` (`noKey: true`) |
